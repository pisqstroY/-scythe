...
do
    local OldLib = pcall(function() return getgenv().FemWare end) and getgenv().FemWare
    if OldLib then
...
local Library do
    ...
    Library = {
        ...
        Folders = {
            Directory = "FemWare",
            Configs = "FemWare/Settings",
            Assets = "FemWare/Assets",
        },
    ...
    Library.Unload = function(self)
        ...
        Library = nil
        pcall(function() getgenv().FemWare = nil end)
        ...
        Components.Dropdown = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["Icon"] = Instances:Create("ImageLabel", {
                    Parent = Items["RealDropdown"].Instance,
                    Name = "\0",
                    ImageColor3 = FromRGB(255, 105, 180),
                    ScaleType = Enum.ScaleType.Fit,
                    BorderColor3 = FromRGB(0, 0, 0),
                    AnchorPoint = Vector2New(1, 0.5),
    ...
    Library.Window = function(self, Data)
        ...
        local Items = Components:Window({
            ...
            Glow.Position = UDim2New(0, -20, 0, -20)
            Glow.Size = UDim2New(1, 40, 1, 40)
            Glow.ZIndex = 0
            Glow.Parent = Items["Window"].Instance

            -- ===== FEMWARE BANNER =====
            local Banner = Instance.new("ImageLabel")
            Banner.Name = "FemBanner"
...
getgenv().FemWare = Library

-- ══════════════════════════════════════════════════════════════════════════════
--  FEMWARE CUSTOM KEYBIND SYSTEM (РОЗОВЫЙ ГЛЯНЦЕВЫЙ СТИЛЬ)
-- ══════════════════════════════════════════════════════════════════════════════
local CoreGui = cloneref and cloneref(game:GetService("CoreGui")) or game:GetService("CoreGui")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
...
WmRoot.Position               = UDim2.new(0, 15, 0.5, -100)
WmRoot.Size                   = UDim2.new(0, 200, 0, 220)
WmRoot.ZIndex                 = 500
WmRoot.Parent                 = KeylistGui

-- РОЗОВОЕ УЛЬТРА-ПЛАВНОЕ СВЕЧЕНИЕ (Слои посажены очень близко друг к другу)
local WmGlowLayers = {}
for i = 8, 1, -1 do
    local s  = i * 1.5 -- Очень маленький шаг, чтобы слои сливались в единый плавный градиент
    local fr = Instance.new('Frame')
    fr.BackgroundColor3       = Color3.fromRGB(255, 105, 180) -- Розовый
    -- Экспоненциальное затухание для эффекта мягкого неонового тумана
    fr.BackgroundTransparency = 1 - (0.05 * math.pow(1.3, 8 - i)) 
    fr.BorderSizePixel        = 0
    fr.Position               = UDim2.new(0, -s, 0, -s)
    fr.Size                   = UDim2.new(1, s * 2, 1, s * 2)
    fr.ZIndex                 = 490 + i
    fr.Parent                 = WmRoot
    Instance.new('UICorner', fr).CornerRadius = UDim.new(0, 5 + s / 2)
...
BlackBox.Position             = UDim2.new(0, 0, 0, 0)
BlackBox.BackgroundColor3     = Color3.fromRGB(20, 20, 20) -- Серый под стиль
BlackBox.BackgroundTransparency = 0.15
BlackBox.BorderSizePixel      = 0
BlackBox.ZIndex               = 501
BlackBox.Parent               = WmRoot

local BoxCorner = Instance.new("UICorner")
BoxCorner.CornerRadius = UDim.new(0, 5)
BoxCorner.Parent = BlackBox

-- Розовая окантовка корпуса
local BoxStroke = Instance.new("UIStroke")
BoxStroke.Thickness = 1
BoxStroke.Color = Color3.fromRGB(255, 105, 180) -- Розовый
BoxStroke.Transparency = 0.4
BoxStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
BoxStroke.Parent = BlackBox

-- Розовый text шапки
local HeaderLabel = Instance.new("TextLabel")
HeaderLabel.Size              = UDim2.new(1, 0, 0, 25)
HeaderLabel.Position          = UDim2.new(0, 0, 0, 6)
HeaderLabel.BackgroundTransparency = 1
HeaderLabel.TextColor3        = Color3.fromRGB(255, 105, 180) -- Розовый
HeaderLabel.Font              = Enum.Font.GothamMedium
HeaderLabel.TextSize          = 12
HeaderLabel.TextXAlignment    = Enum.TextXAlignment.Center
HeaderLabel.Text              = "K E Y B I N D S"
HeaderLabel.ZIndex            = 502
...
Separator.BackgroundColor3 = Color3.fromRGB(255, 105, 180) -- Розовый
Separator.BackgroundTransparency = 0.85
Separator.BorderSizePixel = 0
Separator.ZIndex            = 502
Separator.Parent = BlackBox

-- КОНТЕЙНЕР ДЛЯ СПИСКА БИНДОВ
local BindsContainer = Instance.new("Frame")
BindsContainer.Size = UDim2.new(1, -16, 1, -42)
BindsContainer.Position = UDim2.new(0, 8, 0, 38)
BindsContainer.BackgroundTransparency = 1
BindsContainer.ZIndex = 502
BindsContainer.Parent = BlackBox
...
UIListLayout.Parent = BindsContainer
...
NoneLabel.Text = "none"
NoneLabel.ZIndex = 503
NoneLabel.Parent = BindsContainer

-- ПУЛ ДЛЯ СТРОК
local rowPool = {}
local function getOrCreateRow(index)
    if rowPool[index] then 
        rowPool[index].Visible = true
    ...
    line.Position = UDim2.new(0.075, 0, 1, -1)
    line.BackgroundColor3 = Color3.fromRGB(255, 105, 180) -- Розовый
...
-- ── ИСПРАВЛЕННЫЙ ПЛАВНЫЙ DRAGGABLE СИСТЕМЫ ────────────────────────────────────
local dragging, dragInput, dragStart, startPos
local function updateDrag(input)
    local delta = input.Position - dragStart
    WmRoot.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
end
BlackBox.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        dragging = true
        dragStart = input.Position
        startPos = WmRoot.Position
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then dragging = false end
        end)
    end
end)
BlackBox.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
        dragInput = input
    end
end)
...
-- OPTIMIZED: Обновление 10 раз в сек
...
RunService.Heartbeat:Connect(function()
    ...
    for _, bind in ipairs(KeybindSystem.Binds) do
        ...
        if isToggled or bind.isHeld or menuOpen then
            ...
            local status = isToggled and "[toggled]" or string.format("[%s]", keyName)
            local rowFrame = getOrCreateRow(activeCount)
            
            local textLabel = rowFrame:FindFirstChildOfClass("TextLabel")
            if textLabel then
                textLabel.Text = string.format("%s   %s", bind.name, status)
            end
        end
    end

    NoneLabel.Visible = (activeCount == 0)
end)
