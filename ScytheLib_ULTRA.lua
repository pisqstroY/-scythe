...
local Library = {
    ...
    AccentColorDark = Color3.fromRGB(200,200,200);     -- небесно-голубой акцент
...
Watermark.Size = UDim2.new(0, 500, 0, 22)

Watermark.Font = Library.Font
Watermark.TextSize = 17
Watermark.TextXAlignment = Enum.TextXAlignment.Left
Watermark.TextColor3 = Library.FontColor
Watermark.TextStrokeTransparency = 0.25
...
local function ApplyBackground()
    ...
    local img = workspace:FindFirstChild("ScytheBackground")
    if img and img:IsA("Decal") then
        ...
        BackgroundImage.Image = img.Texture
...
-- UI SETTINGS HOOK (если у тебя есть табы)
pcall(function()
    local VisualsBox = Tabs.Settings:AddLeftGroupbox("Visuals")

    VisualsBox:AddToggle("GlowEnabled", {
        Text = "Glow Enabled",
        Default = true,
        Callback = function(v)
            GlowEnabled = v
    ...
    VisualsBox:AddSlider("GlowSpeed", {
        Text = "Glow Speed",
        Default = 2,
        Min = 0.5,
        Max = 5,
    ...
    VisualsBox:AddButton("Reload Background", function()
        ApplyBackground()
    end)
end)

--// === SCYTHE ULTRA UPGRADE ===
...
function Library:LoadBackgroundFromWorkspace(name)
    if BackgroundImage then
        BackgroundImage:Destroy()
    end

    local obj = workspace:FindFirstChild(name or "ScytheBackground")
    if obj and obj:IsA("Decal") then
        BackgroundImage = Instance.new("ImageLabel")
        BackgroundImage.Parent = game:GetService("CoreGui")
        BackgroundImage.Size = UDim2.new(1,0,1,0)
        BackgroundImage.Position = UDim2.new(0,0,0,0)
        BackgroundImage.BackgroundTransparency = 1
...
-- ⚙️ UI SETTINGS INTEGRATION
task.spawn(function()
    repeat task.wait() until Library and Library.Tabs

    pcall(function()
        local Tabs = Library.Tabs
        if not Tabs or not Tabs["UI Settings"] then return end

        local Visuals = Tabs["UI Settings"]:AddLeftGroupbox("Scythe Visuals")

        Visuals:AddToggle("RainbowAccent", {
            Text = "Rainbow Accent",
            Default = false,
            Callback = function(v)
                RainbowEnabled = v
        ...
        Visuals:AddSlider("RainbowSpeed", {
            Text = "Rainbow Speed",
            Default = 1,
            Min = 0.5,
            Max = 5,
        ...
        Visuals:AddButton("Load Background", function()
            Library:LoadBackgroundFromWorkspace("ScytheBackground")
        end)
    end)
end)

-- ✨ WATERMARK GLOW (НЕ новый, а через accent)
local GlowSpeed = 2
